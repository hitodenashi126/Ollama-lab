...
export default function SettingsModal({ 
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      />
      <div
        className="relative bg-[var(--surface)] backdrop-blur-2xl w-full max-w-3xl rounded-xl border border-[var(--surface-border)] shadow-2xl overflow-hidden flex flex-col md:flex-row h-[min(680px,calc(100dvh-2rem))] shadow-black/20 dark:shadow-black/60 transition-colors duration-300 animate-scale-in"
      >
        {/* Sidebar */}
          ...
          <nav className="flex-1 p-3 space-y-1">
            {categories.map((cat) => (
              <button
                ...
                className={cn(
                  "w-full flex items-center gap-3 px-3 py-2 rounded-lg text-xs font-bold uppercase tracking-widest transition-all active:scale-[0.98]",
        ...
        <div className="flex-1 flex flex-col bg-transparent overflow-hidden">
            ...
            {activeCategory === 'general' && (
              <>
                ...
                <section className="space-y-4">
                  <div className="flex items-center gap-2 text-neutral-500">
                    <Layout className="w-4 h-4" />
                    <h3 className="text-[10px] uppercase font-bold tracking-widest">Chat Typography</h3>
                  </div>
                  <div className="grid grid-cols-3 gap-3">
                    {[
                      { id: 'sans', name: 'Sans', description: 'Modern' },
                      { id: 'serif', name: 'Serif', description: 'Classic' },
                      { id: 'mono', name: 'Mono', description: 'Code-like' },
                    ].map((f) => (
                      <button
                        key={f.id}
                      ...
                      >
                        ...
                        <span className="text-[8px] opacity-70">{f.description}</span>
                      </button>
                    ))}
                  </div>
          ...
          <div className="p-4 md:p-6 bg-black/[0.02] dark:bg-white/[0.02] border-t border-[var(--surface-border)] flex items-center justify-end shrink-0 gap-4">
            <button
              onClick={onClose}
